...
After cleaning the dataset, the script creates a `data` folder if it does not already exist and saves the cleaned dataset as a new CSV or Excel file with `_cleaned` appended to the original filename. The original dataset remains unchanged, allowing users to keep both the original data and the cleaned data. 